    ...
    <script>
      ...
      async function loadNews() {
        try {
          ...
          news.forEach(item => {
            ...
            // News-Card in 3 Spalten: Bild / (Spalte 2: Datum, Titel, Tabs, Buttons) / (Spalte 3: Inhalt)
            card.innerHTML = `
              ...
              <!-- Buttons (optional, falls du sie auch in Spalte 2 haben möchtest, wurden sie bereits dort integriert) -->
            `;
            container.appendChild(card);
          });
      
          // Tab-Event-Listener: Prüfe vor Tabwechsel, ob Editierfelder aktiv sind; ansonsten umschalten
      ...
      function toggleEdit(btn, newsId) {
        ...
        if (!titleInput && !contentTextarea) {
          // Noch nicht editiert: Ersetze statischen Text durch Formularfelder
          ...
          // Neu: Eingabefeld so formatieren, dass es in einer Zeile bleibt
          ...
          // Werte auslesen und via AJAX senden
          ...
          updateNewsData(newsId, lang, newTitle, newContent)
            .then(response => {
              ...
              pContent.style.textAlign = 'left'; // Stil explizit setzen
              pContent.textContent = newContent;
              contentTextarea.replaceWith(pContent);
